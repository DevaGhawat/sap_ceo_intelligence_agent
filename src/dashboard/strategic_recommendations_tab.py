...
def show_strategic_recommendations():
    ...
    if "strategic_question" not in st.session_state:
        st.session_state["strategic_question"] = ""

    if "strategic_result" not in st.session_state:
        st.session_state["strategic_result"] = None
    ...
    if run_button:
        if not question.strip():
            ...
            with st.spinner("Running CEO agent: planning, retrieving, analyzing, validating..."):
                st.session_state["strategic_result"] = run_ceo_agent(question)

    result = st.session_state.get("strategic_result")

    if not result:
        st.info("Enter a strategic CEO-level question and click Generate Strategic Recommendations.")
        return
